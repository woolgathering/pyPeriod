...
class QOPeriods(Periods):
    ...
    def find_periods(
        ...
        **kwargs : dict, optional
        ...
        if n_periods is None:
            n_periods = N  # way too many but whatever
        periods = np.zeros(n_periods, dtype=np.uint32)  # initialize
        possible_periods = np.arange(min_length, max_length + 1)
        norms = np.zeros(n_periods)  # initialize
        res = data.copy()  # copy
        ...
        for i in range(n_periods):  # loop over the number of periods to find
                ...
                if self._orthogonalize:
                    ...
                    if update_weights:
                        ...
                        best_base = self.project(
                            data=res,
                            p=best_p,
                            trunc_to_integer_multiple=self._trunc_to_integer_multiple,
                            orthogonalize=True,
    ...
    def get_periods(self, weights, dictionary, decomp_type="row reduction"):
        ...
        if decomp_type == "row reduction":
            ...
            coeffs, reconstructed = self.solve_quadratic(
                concatenated_periods, A, k=self._k, type="solve"
            )
        elif decomp_type == "lu":
            PL, U = spla.lu(A, permute_l=True)
            coeffs, reconstructed = self.solve_quadratic(
                concatenated_periods, U, k=self._k, type="solve"
            )
        elif decomp_type == "qr":
            Q, R = np.linalg.qr(A, mode="complete")
            coeffs, reconstructed = self.solve_quadratic(
                concatenated_periods, R, k=self._k, type="solve"
            )
        else:
            coeffs, reconstructed = self.solve_quadratic(
                concatenated_periods, A, k=self._k, type="lstsq"
            )
    ...
    def _eq_3(self, x, P):
        ...
        M = N // P
        for l in range(1, M):
            second_term += self._auto_corr(x, int(l * P))
        second_term *= 2
        return fac * (self._auto_corr(x, 0) + second_term)

    def _auto_corr(self, x, k):
        """
